...
class Assign:
    """
    Match the last assignment node in the AST module.
    Expected to match ``result = sum(1, 2)`` but not ``sum(1, 2)``.
    """
    ...
    def get_is_match(self) -> bool:
        ...
        if len(self.ast_mod.body) < 1:
            return False
        is_instance = isinstance(self.ast_mod.body[-1], (ast.Assign, ast.AnnAssign))
