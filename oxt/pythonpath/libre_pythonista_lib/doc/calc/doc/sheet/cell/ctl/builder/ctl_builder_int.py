...
if TYPE_CHECKING:
    ...
    from oxt.pythonpath.libre_pythonista_lib.cq.cmd.calc.sheet.cell.ctl.create.cmd_int import CmdInt
    ...
    from libre_pythonista_lib.cq.cmd.calc.sheet.cell.ctl.create.cmd_int import CmdInt
...
class CtlBuilderInt(CtlBuilder):
    @override
    def append_commands(self) -> None:
        ...
        self.append(CmdRuleName(cell=self.cell, ctl=self.ctl, kind=RuleNameKind.CELL_DATA_TYPE_INT))
        self.append(CmdOrigRuleName(cell=self.cell, ctl=self.ctl, kind=RuleNameKind.CELL_DATA_TYPE_INT))
        self.append(CmdArrayAbility(cell=self.cell, ctl=self.ctl, ability=False))
        self.append(CmdModifyTriggerEvent(cell=self.cell, ctl=self.ctl, kind=RuleNameKind.CELL_DATA_TYPE_INT))
        self.append(CmdInt(cell=self.cell, ctl=self.ctl, opt=self._get_control_options()))
