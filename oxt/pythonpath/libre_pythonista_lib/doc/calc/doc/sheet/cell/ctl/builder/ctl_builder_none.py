...
if TYPE_CHECKING:
    ...
    from oxt.pythonpath.libre_pythonista_lib.cq.cmd.calc.sheet.cell.ctl.create.cmd_none import CmdNone
    ...
    from libre_pythonista_lib.cq.cmd.calc.sheet.cell.ctl.create.cmd_none import CmdNone
...
class CtlBuilderNone(CtlBuilder):
    @override
    def append_commands(self) -> None:
        ...
        self.append(CmdRuleName(cell=self.cell, ctl=self.ctl, kind=RuleNameKind.CELL_DATA_TYPE_NONE))
        self.append(CmdOrigRuleName(cell=self.cell, ctl=self.ctl, kind=RuleNameKind.CELL_DATA_TYPE_NONE))
        self.append(CmdArrayAbility(cell=self.cell, ctl=self.ctl, ability=False))
        self.append(CmdModifyTriggerEvent(cell=self.cell, ctl=self.ctl, kind=RuleNameKind.CELL_DATA_TYPE_NONE))
        self.append(CmdNone(cell=self.cell, ctl=self.ctl, opt=self._get_control_options()))
