...
class PythonCode:
    ...
    def _is_py_formula(self, formula: Any) -> bool:  # noqa: ANN401
        with self._log.indent(True):
            ...
            if not isinstance(formula, str):
                self._log.debug("_is_py_formula: formula is not str")
                return False
    ...
    def get_formula_addr(self) -> Union[CellAddress, None]:
        with self._log.indent(True):
            ...
            if sel is None:
                self._log.debug("get_formula_addr: sel is None")
                with contextlib.suppress(Exception):
                    ...
                    if self._is_py_formula(cell.getFormula()):
                        self._log.debug("get_formula_addr: got cell from doc.get_selected_cell_addr()")
                        return addr
                self._log.debug("get_formula_addr: return None")
                return None

            # is this cell selection?
            name = sel.getImplementationName()
            if name == "ScCellObj":
                self._log.debug("get_formula_addr: cell selection is ScCellObj")
                sel_cell = cast(ScCellObj, sel)
                if not self._verify_is_formula:
                    self._log.debug("get_formula_addr: return sel_cell.getCellAddress() without verifying formula")
                    return sel_cell.getCellAddress()  # type: ignore
                formula = sel_cell.getFormula()  # type: ignore
                if not formula:
                    self._log.debug("get_formula_addr: sel_cell.getFormula() is empty")
                    return None
                if self._is_py_formula(formula):
                    self._log.debug("get_formula_addr: return sel_cell.getCellAddress()")
                    return sel_cell.getCellAddress()  # type: ignore
            if name == "ScCellRangeObj":
                self._log.debug("get_formula_addr: cell selection is ScCellRangeObj")
                sel_cells = cast(ScCellRangeObj, sel)
                if not self._verify_is_formula:
                    self._log.debug("get_formula_addr: return sel_cells.getRangeAddress() without verifying formula")
    ...
    def _get_file_name(self) -> str:
        if not self._addr:
            self._log.error("_get_file_name: No cell selected")
        ...
        with self._log.indent(True):
            ...
            try:
                ...
                if len(sheet.draw_page.forms) == 0:
                    self._log.debug("_get_sheet_id: No forms found. Creating a new Form")
                    frm = sheet.draw_page.forms.add_form("Form1")
                else:
                    self._log.debug("_get_sheet_id: Form found")
