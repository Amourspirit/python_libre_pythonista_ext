...
if TYPE_CHECKING:
    ...
    from oxt.pythonpath.libre_pythonista_lib.cq.cmd.calc.sheet.cell.ctl.create.cmd_data_tbl import CmdDataTbl
    ...
    from libre_pythonista_lib.cq.cmd.calc.sheet.cell.ctl.create.cmd_data_tbl import CmdDataTbl
...
class CtlBuilderDataTbl(CtlBuilder):
    @override
    def append_commands(self) -> None:
        self.append(CmdLblRR(cell=self.cell, ctl=self.ctl, res_key="ctl003"))
        self.append(CmdControlName(cell=self.cell, ctl=self.ctl))
        self.append(CmdCtlShapeName(self.cell, self.ctl))
        self.append(CmdRuleName(cell=self.cell, ctl=self.ctl, kind=RuleNameKind.CELL_DATA_TYPE_TBL_DATA))
