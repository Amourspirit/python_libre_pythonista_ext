...
def check_breakpoint(label: str):  # noqa: ANN201
    ...
    The decorator initializes an instance of ``LpBreakMgr`` and checks if a debugger is attached.
    If a debugger is attached, it prints a message indicating the label of the breakpoint being checked
    and calls the ``check_breakpoint`` method of ``LpBreakMgr`` with the provided label.
