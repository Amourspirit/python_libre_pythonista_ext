...
class CtlMgr:
    ...
    def get_current_ctl_type_from_cell(self, cell: CalcCell) -> Type[SimpleCtl] | None:
        ...
        km = self._key_maker
        try:
            self._log.debug(f"CtlMgr - get_current_ctl_type_from_cell() Getting control type for cell {cell.cell_obj}")
    ...
    def get_orig_ctl_type_from_cell(self, cell: CalcCell) -> Type[SimpleCtl] | None:
        ...
        km = self._key_maker
        try:
            self._log.debug(f"CtlMgr - get_orig_ctl_type_from_cell() Getting control type for cell {cell.cell_obj}")
            ...
            ctl_type = self._get_rule(rule_name, cell)
            if ctl_type:
                self._log.debug(f"CtlMgr - get_orig_ctl_type_from_cell() Found control type for cell {cell.cell_obj}")
            else:
                self._log.debug(
