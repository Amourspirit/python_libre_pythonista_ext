...
#         self.trigger_event("PySourceManagerCreated", EventArgs(self))
#         self._is_init = True
...
#             #     return src_code
#             # except Exception:
#             #     self.log.error("PySourceManager - dump_module_source_code_to_log() - Error dumping to LpLog log.")
#             return src_code
...
#         if is_db:
#             with self.log.indent(True):
#                 self.log.debug(f"__getitem__() - Code Cell: {code_cell}")
#         result = self._data[code_cell]
#         if is_db:
#             with self.log.indent(True):
#                 self.log.debug(f"__getitem__() - Result Unique Id: {result.unique_id}")
#         return result
...
#             row = code_cell[1]
#             col = code_cell[2]
#             self.log.debug(f"add_source() sheet index: {sheet_idx} col: {col}, row: {row}")
#             if code_cell in self._data:
#                 self.log.error(f"add_source() - Cell {cell} already exists.")
...
#             self.log.debug(f"add_source() - Adding Source ID: {str_id}")
...
#             self.log.debug(f"add_source() - Setting custom property: {km.cell_addr_key} to {addr}")
...
#             self.log.debug(f"add_source() - inserting for cell {cell}: sheet index: {sheet_idx}")
#             cc.insert(cell=cell, code_name=str_id, props={cc.code_prop}, sheet_idx=sheet_idx)
#             self.log.debug(f"add_source() - inserted for cell {cell}: sheet index: {sheet_idx}")
#             # CellCache.reset_instance()
#             self._data[code_cell] = py_src
#             index = self.get_index(cell)
#             if index < 0:
#                 self.log.error(f"add_source() - Cell {cell} not found.")
#                 raise Exception(f"Cell {cell} not found.")
#             if self._is_last_index(index):
#                 self.log.debug(f"add_source() - Last Index, updating from index {index}")
#                 self.update_from_index(index)
#             else:
#                 self.log.debug(f"add_source() not last index, Updating all")
#                 self.update_all()
...
#             row = code_cell[1]
#             col = code_cell[2]
#             self.log.debug(f"update_source() sheet index: {sheet_idx} col: {col}, row: {row}")
...
#             index = self.get_index(cell)
#             if index < 0:
#                 self.log.error(f"update_source() - Cell {cell} not found.")
#                 raise Exception(f"Cell {cell} not found.")
#             src.source_code = code  # writes code to file
#             # CellCache.reset_instance()

#             if self._is_last_index(index):
#                 self.log.debug(f"update_source() is last index updating from index {index}")
#                 self.update_from_index(index)
#             else:
#                 self.log.debug(f"update_source() not last index, Updating all")
#                 self.update_all()
#             eargs = EventArgs.from_args(cargs)
...
#             row = code_cell[1]
#             col = code_cell[2]
#             self.log.debug(f"remove_source() sheet index: {sheet_idx} col: {col}, row: {row}")
...
#                 self.log.debug(f"remove_source() Has custom property: {cc.code_prop}. Removing ...")
#                 calc_cell.remove_custom_property(cc.code_prop)
#                 self.log.debug("remove_source() custom property removed.")
...
#             self.log.debug(f"remove_source_by_calc_cell() - Cell is deleted. {cell.cell_obj}")
...
#                 self.log.debug(f"remove_source_by_calc_cell() - Deleted file: {uri}")
#             else:
#                 self.log.debug(f"remove_source_by_calc_cell() - File not found: {uri}")
...
#         """
#         with self.log.indent(True):
#             self.log.debug(f"set_global_var() - Setting Global Variable: {name} = {value}")
...
#             except Exception:
#                 self.log.warning(f"get_index() - Cell {cell} not found.")
#                 return -1
...
#         """
#         with self.log.indent(True):
#             self.log.debug(f"update_from_index({index}) Entered.")
#             length = len(self)
#             if index >= length:
...
#             py_src = self[co]
#             if self._is_last_index(index):
#                 self.log.debug(f"update_from_index({index}). Is last index.")
#             else:
#                 self.log.debug(f"update_from_index({index}). Is not last index. Resetting module to py_src dict.")
...
#                 self._update_item(py_src)
#             self.log.debug(f"update_from_index({index}) Leaving.")
