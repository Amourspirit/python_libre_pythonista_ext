...
class QryHandler(QryHandlerT):
    def handle(self, query: QryT) -> Any:  # noqa: ANN401
        if query.kind in (CalcQryKind.SIMPLE, CalcQryKind.CELL, CalcQryKind.SHEET):
            ...
            return self._handle_cell_cache(cast(QryCellCacheT, query))
        elif query.kind == CalcQryKind.SHEET_CACHE:
            return self._handle_sheet_cache(cast(QrySheetCacheT, query))
        else:
            raise NotImplementedError

    def _handle_simple(self, query: QryT) -> Any:  # noqa: ANN401
        return query.execute()

    def _handle_cell_cache(self, query: QryCellCacheT) -> Any:  # noqa: ANN401
    ...
    def _handle_sheet_cache(self, query: QrySheetCacheT) -> Any:  # noqa: ANN401
