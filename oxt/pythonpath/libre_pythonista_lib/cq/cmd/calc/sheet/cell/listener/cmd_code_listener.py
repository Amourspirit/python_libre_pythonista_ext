...
class CmdCodeListener(CmdBase, LogMixin, CmdCellT):
    """
    Command to add a listener to a cell.
    ...
    def execute(self) -> None:
        """
        Executes the command to add a listener to the cell.
        ...
        try:
            if self._current_has_listener:
                self.log.debug("Listener already exists for cell %s", self.cell.cell_obj)
            else:
                cell_listener_result = listeners.add_listener(self.cell)
                if Result.is_failure(cell_listener_result):
                    self.log.error("Failed to add listener for cell %s", self.cell.cell_obj)
                    return

                self.log.debug("Successfully added listener for cell %s", self.cell.cell_obj)
    ...
    def _undo(self) -> None:
        """Internal method to undo the last operation"""
        try:
            ...
            if TYPE_CHECKING:
                ...
                from libre_pythonista_lib.cq.cmd.calc.sheet.cell.listener.cmd_code_listener_del import (
                    CmdCodeListenerDel,
                )
            cmd = CmdCodeListenerDel(cell=self.cell)
