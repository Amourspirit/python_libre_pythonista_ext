...
class QryLpCellsLast(QryBase, LogMixin, QrySheetT[Result[CellObj, None] | Result[None, Exception]]):
    """Query that returns the last LibrePythonista cell found in a sheet."""
    ...
    def execute(self) -> Result[CellObj, None] | Result[None, Exception]:
        """
        Execute the query to get the last LibrePythonista cell.

        Returns:
            Result: Success with last CellObj found, or Failure with None/Exception if no cells exist or on error.
        """
        try:
