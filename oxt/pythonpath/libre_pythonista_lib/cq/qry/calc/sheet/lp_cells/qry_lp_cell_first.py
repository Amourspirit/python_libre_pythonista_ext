...
class QryLpCellFirst(QryBase, LogMixin, QrySheetT[Union[Result[CellObj, None], Result[None, Exception]]]):
    """
    Query to get the first LibrePythonista cell in a sheet.

    Args:
        sheet (CalcSheet): The sheet to query.
        from_sheet (bool, optional): If True, gets cells directly from sheet. If False, gets cells from Python source manager. Defaults to False.
    """
    ...
    def _execute_from_py_src_mgr(self) -> Union[Result[CellObj, None], Result[None, Exception]]:
        ...
        Returns:
            Result: Success with first CellObj or Failure with None/Exception.
        """
        try:
    ...
    def _execute_from_sheet(self) -> Union[Result[CellObj, None], Result[None, Exception]]:
        ...
        try:
            ...
            # return last item in sheet_cells
            return Result.success(sheet_cells[0])
        except Exception as e:
            self.log.exception("Error executing query")
            return Result.failure(e)
